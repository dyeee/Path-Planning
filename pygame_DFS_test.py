# -*- coding: utf-8 -*-
"""
Created on Wed Mar 12 16:06:54 2025
...
# 初始化 Pygame
pygame.init()
wind = pygame.display.set_mode([1000, 800])  # 加寬視窗以顯示統計數據
pygame.display.set_caption("DFS 迷宫求解 - 探索模式")
...
# 判斷是否合法
def isok(x, y, board, visited):
    return 0 <= x < len(board) and 0 <= y < len(board[0]) and board[x][y] != 0 and (x, y) not in visited

# DFS 探索 & 找到終點
def explore_dfs(start, board, hidden_goal):
    stack = [(start[0], start[1], [], 0)]  # (x, y, 路徑, 步數)
    visited = set()
    discovered_goal = False  
    goal_position = None  

    while stack:
        x, y, path, steps = stack.pop()
        visited.add((x, y))
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:  
            nx, ny = x + dx, y + dy
            if isok(nx, ny, board, visited):
                stack.append((nx, ny, path + [[x, y]], steps + 1))
...
def test_mode(num_tests=100):
    ...
    for _ in range(num_tests):
        ...
        for start in start_positions:
            explored_path, found_goal, steps = explore_dfs(start, board, hidden_goal)
            ...
            draw_map(board, goal_position, explored_path, discovered=discovered_goal, last_steps=last_steps)
            time.sleep(0.3)  
